...
url = BASE_URL  + zbotID + "/fastscan/attendance/548/report"
# url = BASE_URL + "library/reading/" + zbotID + "/report"
method = "POST"

# /:version/:ZviceID/fastscan/attendance/:FSCardID/report   POST
# numdays // integer

body = {"numdays" : 1}
jsonreply = hit_url_method(body, headers1, method, url)

jsonreply = json.loads(jsonreply)['data']['elements']
# jsonreply = json.loads(jsonreply)
for elements in jsonreply:
    print elements['content']
matches = re.findall('\w{13}', elements['content'], re.DOTALL)
print(matches)
...
recipients = ['sujoy@silabtech.com']
sub = "reports"
