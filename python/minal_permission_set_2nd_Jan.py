...
def set_card_permissions_set(cardID, zviceID, acttype, headers1, ZbotID, BASE_URL,policyval):
    ...
    response1 = change_view_permissions_fullurl(RequestBody, headers1, url)
    return response1

def set_card_permissions_notifi(cardID, zviceID, acttype, headers1, ZbotID, BASE_URL,sendMail,sendNotification):
    ...
    response2 = change_view_permissions_fullurl_notifi(RequestBody, headers1, url)
    return response2

def change_view_permissions_fullurl_notifi(body, headers, BASE_URL):
        ...
        for row in data:
                ...
                if title in a['title']:
                    ...
                    for ac in json.loads(ja)['data']['elements']:
                        if "Monday" in ac['title']:
                            ...
                            cardID = data1['formID']
                            response = set_card_permissions("Counsellors", cardID, zviceID, "VIEW", headers1,"8SFBUKFZCALEE", BASE_URL)
                            print response

                            response3 = set_card_permissions("Linked Users", cardID, zviceID, "MAIL", headers1,"8SFBUKFZCALEE", BASE_URL)
                            print response3

                            response4 = set_card_permissions_notifi(cardID, zviceID, "MAIL", headers1, "8SFBUKFZCALEE", BASE_URL, "true", "true")
                            print response4

                            response1 = set_card_permissions_set(cardID, zviceID, "AUTO_UPDATE_CARD_MAIL", headers1,"8SFBUKFZCALEE", BASE_URL, "true")
                            print response1
                                ...
                                if title in subac['title']:
                                    ...
                                    for subac1 in subac['actions']:
                                        ...
                                        if title in subac1['title']:
                                            ...
                                            pid = data1['cardID']
                                            print pid
                                            response2 = set_card_permissions("Linked Users", pid, zviceID, "ALLOWED_USERS",headers1, "8SFBUKFZCALEE", BASE_URL)
                                            print response2
            #
                        if "Tuesday" in ac['title']:
                            print "2nd level"
                            data1 = json.loads(ac['content'])
                            cardID = data1['formID']
                            response = set_card_permissions("Counsellors", cardID, zviceID, "VIEW", headers1,"8SFBUKFZCALEE", BASE_URL)
            ...
            for a in jsondata11['data']['elements']:
                ...
                if title in a['title']:
                    ...
                    for ac in json.loads(ja)['data']['elements']:
                        if "Body Measurements" in ac['title']:
                            print "2nd level"
                            data1 = json.loads(ac['content'])
                            cardID = data1['formID']
                            response = set_card_permissions("Counsellors", cardID, zviceID, "VIEW",headers1, "8SFBUKFZCALEE", BASE_URL)
                            ...
                            response5 = set_card_permissions("Linked Users", cardID, zviceID, "VIEW", headers1, "8SFBUKFZCALEE", BASE_URL)
                            print response5
                        ...
                        if title in subac['title']:
                            ...
                            for subac1 in subac['actions']:
                                ...
                                if title in subac1['title']:
                                    ...
                                    pid = data1['cardID']
                                    print pid
                                    response = set_card_permissions("Linked Users", pid, zviceID, "ADMIN", headers1,"8SFBUKFZCALEE", BASE_URL)
                                    print response

                # if "Emergency Contact" in
...
### not here - hit_any_url
    ...
    # response = set_card_permissions("Counsellors", "1881", "9YZ2HAZE4B5TN", "VIEW", headers1, "8SFBUKFZCALEE", BASE_URL)
                            ## great - now please go to the xls
