        ...
        for f_name in result['data']['elements']:
            if "Project Initiation" in f_name['title']:
                ...
                for e1 in data['Elements'][0]['Elements']:
                    if "Project Name" in e1['FieldLabel']:
...
def create_worflow_elements(wfe, body, wfesparentID, filepath, filepath_server, BASE_URL, headers1, threadID, threads, wfectr, cardList, top_status,optional = None):
    #
    ...
    val1 = {}
    try:
        val1 = WP.parse_wfe_ordered(filename,optional=optional)
    except:
        val1 = WP.parse_wfe_ordered(filename_server,optional=optional)
    ...
    if optional == None:
        ...
        result = CM.create_txt_card(val1['desc'], "", body['BusinessTag'], headers1, wfesparentID, BASE_URL, "false","true")
        CardIds = str(result)
        metaDATA = {"parallelWFE" : "true"}
        CM.textCard_metadata(CardIds, metaDATA, BASE_URL, body['BusinessTag'], headers1)
        tagName = body['WorkflowID']
        newtag = val1['subFlow'] + ":WFID_" + str(tagName) + ", wfe:" + val1['name'] + "," + "parallelWFE_true"
        result = CM.add_tags_future(CardIds, newtag, BASE_URL, body['BusinessTag'], headers1, body['WorkflowID'])
        CM.create_txt_card("dummy", "dummy", body['BusinessTag'], headers1, CardIds, BASE_URL, "true", "false")
    ...
    for ele in usualForms:
        ...
        create_usual_forms(val1, ele, body, BASE_URL, headers1, wfectr,optional = optional)
        ...
        for k, v in item.items():
            ...
            try:
                try:
                    ...
                    fieldsmeta = json.dumps({"HideEditAction": True})
                result_1 = CM.form_submission_using_NEW_API_meta(BASE_URL, body['BusinessTag'], headers1, cardList[form_key],item[k]['fields'], fieldsmeta)  # submitting form
            except:
            ...
            if optional == None:
                ...
                cardtag = val1['name']+ ":" + optional + ":WFID_" + str(tagName) + "," + k
                logging.warning("**************************************")
                logging.warning(cardtag)
                logging.warning("************************************")
                tagName = val1['name'] + ":" + optional + ":" + val1['subFlow'] + ":WFID_" + str(tagName) + "," + k
                taskTagname = "task:" + tagName
                CM.add_tags_future(s_id, cardtag, BASE_URL, body['BusinessTag'], headers1, body['WorkflowID'])
    ...
    try:
        ...
        for tag1 in tagnames:
            ...
            if len(tag2) == 4:
                ...
                logging.warning(wfid_keyVal)

            elif len(tag2) == 5:
                toptag = "TOP:" + tag2[4]
                wfid_keyVal = tag2[4].split("_")
                wfid_keyVal = wfid_keyVal[1]
                logging.warning("TASK")
                logging.warning(wfid_keyVal)
...
def create_user_grps(body, flow, BASE_URL, headers1, parentID,optional = None):
    tagName = body['WorkflowID']
    tagNameTop = "WFID_" + str(tagName)
    grpname = flow + "p_" + str(tagName)
    mgrgroup = flow + "mgr_" + str(tagName)
    try:
        newgrpID = CM.create_groups(grpname, flow + " group for project : " + body['FormData']['Project Name'], headers1, body['BusinessTag'], BASE_URL)
        mgrgrpID = CM.create_groups(mgrgroup, flow + " group for project : " + body['FormData']['Project Name'],headers1, body['BusinessTag'], BASE_URL)
    except:
...
def create_usual_forms(val1, ele, body, BASE_URL, headers1, wfectr,optional = None):
    if optional == None:
        tagName1 = val1['name'] + ":WFID_" + str(body['WorkflowID']) + "," + ele
    else:
        tagName1 = val1['name'] + ":" + optional + ":WFID_" + str(body['WorkflowID']) + "," + ele
    form_key = ele
...
def create_wfes(body, masterfileName, headers1, BASE_URL, optional = None):
    ...
    try:
        for flow in lisval['subflows']:
            ...
            # threadname.start()
            # threads.append(threadname)
            create_user_grps(body, flow, BASE_URL, headers1, parentID, optional = optional)
    except:
    ...
    try:
        ...
        for formname in result['data']['elements']:
            if "Project Initiation" in formname['title']:
    ...
    for wfe in lisval['wfes']:
        ...
        threads = create_worflow_elements(wfe, body, wfesparentID, filepath, filepath_server, BASE_URL, headers1, threadID, threads, wfectr, cardList, top_status,optional = optional)
    ...
    for wfe in lisval['deptwfes']:
        ...
        if optional == None:
            ...
            create_config_cards_optional(wfe, lisval, wfesparentID, parentID, filepath, filepath_server, body, BASE_URL, headers1, optional = optional)
...
def create_config_cards_optional(wfe, lisval, wfesparentID, parentID, filepath, filepath_server, body, BASE_URL, headers1, optional = None):
    ...
    for per in config_permission:
        CM.set_card_permissions(BASE_URL, per, CardIds, body['BusinessTag'], "VIEW", headers1)
    ...
    grpname = optional + "p_" + str(tagName)
    newgrpID = CM.create_groups(grpname, optional + " group for project " ,headers1, body['BusinessTag'], BASE_URL)
    ...
    for ele in lisval['wfes']:
        ...
        if wfe_1 in name:
            ...
            newtag = "task:" + name + ":" + optional + ":" + wfe + ":WFID_" + str(body['WorkflowID'])
            if tagT == "":
                tagT = newtag
            else:
                tagT = tagT + "," + newtag
    ...
    grp_name = optional + "p_" + str(body['WorkflowID'])
    grp_ID = CM.find_out_grp_ID(grp_name, headers1, body['BusinessTag'], BASE_URL)
    ...
    for deptTasks in alltasks:
        ...
        for tag in deptTasks['tags']:
            ...
            if (re.search(r'task:(.*):\w+:', tag['TagName'])):
                ...
                wfename = wfename.split(":")
                wfename = wfename[0]
                wfename = wfename.split("_")
                wfename = wfename[1] + "_" + wfename[2] + "_" + wfename[3]
    ...
    for ele in lisval['wfes']:
        ...
        if wfe_1 in name:
            ...
            try:
                ...
                val1 = WP.parse_wfe(filename_server)
                desc = val1['desc']
            tag = val1['name'] + ":" + optional + ":WFID_" + str(body['WorkflowID'])
            result1 = CM.get_all_tagIds(tag, BASE_URL, body['BusinessTag'], headers1)
            result = json.loads(CM.get_all_tagIds(tag, BASE_URL, body['BusinessTag'], headers1))
...
def create_config_cards(wfe, lisval, wfesparentID, parentID, filepath, filepath_server, body, BASE_URL, headers1, optional = None):
...
def electrical_submit(body, BASE_URL, headers1, Electical_subflow):
    if len(body['tags']) > 0:

        logging.warning("@@@@@@@@@@@@@@@@@@@@@@@@@")
        logging.warning(datetime.datetime.now())
        logging.warning("@@@@@@@@@@@@@@@@@@@@@@@@@")
        input_data = body['FormData']['Type']
        ...
        subflow_name = body['FormData']['Subflow Name']
        result = CM.create_txt_card((str(wfid) + "wfes"), "workflow_id container", body['BusinessTag'], headers1, "",BASE_URL, "true", "false")
        wfesparentID = str(result)
        CM.hide_card(headers1, BASE_URL, body['BusinessTag'], (str(wfid) + "wfes"))

        t_cardID = CM.create_txt_card(str(subflow_name), "", body['BusinessTag'], headers1, wfesparentID, BASE_URL,"false", "true")
        newtag = "Electrical:WFID_" + str(wfid) + "," + "wfe:Electrical_" + str(subflow_name)
        result = CM.add_tags_future(t_cardID, newtag, BASE_URL, body['BusinessTag'], headers1, wfid)
        CM.create_txt_card("dummy", "dummy", body['BusinessTag'], headers1, t_cardID, BASE_URL, "true", "false")

        if "Type A" in body['FormData']['Type']:
            # masterFileName = "WFE_subflow_electrical_1.csv"

            arr = ["Sub Flows", "WFEs"]
            filename = filepath_server + "WFE_subflow_electrical_1.csv"
            # filename = filepath + "WFE_subflow_electrical_1.csv"
            val_1 = WP.parse_wfe_list(filename)
            ...
            for elm in val_1['deptwfes']:
                elm1 = elm.split("_")
                dept_name = elm1[0] + "_" + subflow_name
                arr.append(dept_name)

            filename = filepath_server + subflow_name + ".csv"
            # filename = filepath + subflow_name + ".csv"
            with open(filename, 'w') as wf:
                for add in arr:
                    wf.write(add + "\n")
            masterFileName = subflow_name + ".csv"
            create_wfes(body, masterFileName, headers1, BASE_URL, optional = subflow_name)

        elif "Type B" in body['FormData']['Type']:
            arr = ["Sub Flows", "WFEs"]
            filename = filepath_server + "WFE_subflow_electrical_2.csv"
            val_1 = WP.parse_wfe_list(filename)
...
def topformSubmit(body, workflowTopFormID, headers1, BASE_URL):
    ...
    if body['FormData']['Project Name'] is None:
        ...
        try:
            ...
            if createdvalue == "113":
                logging.warning("resubmit")
                #
                masterFileName = "haldiram_wfe.csv"
                create_wfes(body, masterFileName, headers1, BASE_URL)
                ...
                if body['FormData']['Start Project?'] == "Yes":
            ...
            try:
                ...
                for inside_group in lead:
                    usergroups = CM.getAllUserGroups(headers1, body['BusinessTag'], BASE_URL, inside_group)
                    grpvals = json.loads(usergroups)
                    grps = grpvals['output']
                    grpname = grps['usergroup']
                    dict1 = grpname
                    grpID = dict1[inside_group]
                    grp_in_grp_ID = str(grpID)
...
def mainworkflow(body, h1, B_URL):
    ...
    filepath_server = "/var/www/cgi-bin/workflow/Haldiram/"
    filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/New_Haldiram_27_FEB/wfe/"
    # filepath_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/New_Haldiram_27_FEB/wfe/"

    workflowTopFormID = "2"     ## Top Project management form TEST SERVER form ID
    create_user_formID = "5849"  #### user addition
    Electical_subflow = "7519"  ### special for haldiram subflow
    ...
    if body['Cmd'] == "form-submit":
        ...
        if body['FormID'] == Electical_subflow:
            electrical_submit(body, BASE_URL, headers1, Electical_subflow)
    ...
    if body['Cmd'] == "textcard-click":
        if "parallelWFE_true" in body['Tags']:
