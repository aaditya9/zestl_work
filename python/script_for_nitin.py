...
#
# form_ID = ""
# input_data = {}     # put this in key, value pair -  Key is field label
# result = CM.form_submission_using_NEW_API(BASE_URL,zviceID,headers1,form_ID,input_data)
# print result
...
def find_skip_in_nextFlow(depTagname, BASE_URL, zviceID, headers1, skipTag,wfid, nxtstages):
    ...
    except:logging.exception("No content found")
...
#     nextstages = result.split(";")
    try:
        ...
        logging.exception("no contents found")
...
#****************************************************************************
        ...
        logging.exception("sending of completion message failed")
    ...
    for item in mul_forms:
        for k, v in item.items():
            ...
            try:
                ...
                logging.exception("This is an illegal form submission " + form_key)
            s_id = str(result)
...
def create_wfes(body, masterfileName, headers1, BASE_URL):
    ...
    try:
        ...
        logging.exception("No subflow was defined")
            ...
            try:
                ...
                for person in lead:
                    if person == '':
                        ...
                        result = CM.add_user_to_group(g_ID, person, body['BusinessTag'], headers1, BASE_URL)
                        logging.warning(result)
            except:logging.exception("adding fail in leads to TOP grp")
        ...
        if body['FormID'] == packagingform:
            ...
            try:
                ...
                try:
                    ...
                    logging.exception("Nothing to do here")
            ...
            try:
                        ...
                        if "Task Assignment Form" in alltag['allTags'] and "Task Assignment Form" in formname:
                                    ...
                                    if "Task Assignment Form" in alltag['allTags'] and "Task Assignment Form" in formname:
                                        ...
                                        for subelm_1 in data['Elements'][0]['Elements']:
                                            try:
                                                ...
                                                logging.exception("no user grp assigned")
                            else:
                                for subelm in data['Elements'][0]['Elements']:
                                    try:
                                        g_ID = subelm['Value']
                            ...
                            for group in uniqgrps:
                                try:
                                    ...
                                    logging.exception("group couldnt be added")
                ...
                #******************************************************************************************************************************************************************
                    ...
                    #             checkboxes[key] = value
                        ...
                        logging.exception("No checkboxes found")
                    flag = False
                    try:
                        ...
                        if flag == False:
                            ...
                            try:
                                ...
                                logging.exception("no tasks found")
                        ...
                        logging.exception("Nothing to do in this form")
            except:
                logging.exception("Not a tagged form")
