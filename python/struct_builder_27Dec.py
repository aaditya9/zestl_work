...
def move_last(tagID, title, BASE_URL, headers1):
    ...
    # print jsondata['data']
    title = unicode(title, 'cp1252')
    custcard = copy.deepcopy(jsondata['data']['elements'])

    for element in jsondata['data']['elements']:
        if title in element['title']:
            # jsondata['data']['elements'].remove(element)
            # element['hidden'] = False
            custcard.append(custcard.pop(custcard.index(element)))
    for element in custcard:
        if title in element['title']:
    # jsondata['data']['elements'].remove(element)
            element['hidden'] = False
    print custcard
...
def returnBaseLevel(columns):
    ...
    for i in base_column:
        ...
        i = columns[0][0]
        if re.search("Base Card", i, re.IGNORECASE):
            print "Base Card found"
            base_card = True
        else:
            sys.exit("Base card is not defined on row[0] col[0]. This is necessary")

        rownum += 1
        i = columns[0][rownum]

        m = re.search(r'\s*(\w{13}).*\(tag\)', i, re.IGNORECASE)
        if m == None:
            sys.exit("Failure to specify a valid base card tag")
        else:
            zbotReturn = m.group(1)
            structure['zbotID'] = zbotReturn
        ...
        try:
            i = columns[colNo][rownum]
        except:
            sys.exit("end of file reached")
        if i == "" and colNo == 0:
            sys.exit("end of file reached")
        if deptFound:
            for i in range (rownum,):
                line = columns[colNo][rownum]
                rownum += 1

                if re.match('hier', line.strip(), re.IGNORECASE):
                    colNo += 1
                elif re.match('end', line.strip(), re.IGNORECASE):
                    deptFound = False
                    depNum += 1

        if (re.search("Department card", i, re.IGNORECASE)):
            print "Found a department"
            deptFound = True
            depNeeds = 2
        if re.search(r'(.*)\(title\)', i, re.IGNORECASE):

            if re.match('end', i, re.IGNORECASE):
                if cardStarted:
                    cardStarted = False
                    if deptFound:
                        deptFound = False
                    ...
                    sys.exit("Orphan END statement found on line " + str(rownum))
        if re.match('hier', i, re.IGNORECASE):
            if cardStarted:
                cardStarted = False
                if deptFound:
                    print "hierarchy inside department detected"
                    # deptFound = False
                    # depNum += 1
                else:
                    cardNum += 1
                ...
                sys.exit("Orphan HIER statement found on line " + str(rownum))
...
def returnBaseLevel(base_column, zbotID):
    baseset = 0
    cardType = [None] * 40
    numcards = 0
    cardfound = 0
...
def createStructure(infile, BASE_URL, headers1, zbotID) :
        ...
        for i in range(1, len(orderedCards)):
            ...
            if (details['level'] == kl):
                ...
                if "department" in details['Ctype'].lower():
                    ...
                    deptTags.append(details['zviceID'])

                    print "======department created========"
                    print "+++++Now Configuring Department++++++++"
                    print unhide_card(details['Ptag'], details['title'], BASE_URL, headers1)
                    print halfwide_card(details['Ptag'], details['title'], BASE_URL, headers1)
                ...
                if "text" in details['Ctype'].lower():
                    ...
                    if cardexists == 0:
                        ...
                        if details['depth'] == 1:
                            try:
                                ...
                                desc1 = " "
                            body = {"FormID": "", "FormDescription": desc1, "FormTitle": details['title'],
                    ...
                    if cardexists == 0:
                        ...
                        if details['depth'] == 1:
                            ...
                            body = {"Text": details['title'], "parentCardID": parentUID, "Flags": "true",
                                    "FlagsInside": "true"}
...
if __name__ == '__main__':
    ...
    # infile = "/home/ec2-user/scripts/input_files/premier_aircon/CardStructure_PremierAirconditioners.csv"
    infile = "C:/Users/User/Dropbox/Zestl-scripts/millennium/script_inputs/MINAL_test_struct.csv"
    ...
    createStructure(infile, BASE_URL, headers1, zbotID)
...
#
# print deptTags
# for tagID in deptTags:
#     # tagID = details['zviceID']
#     title = "Contact Details"
#     hide_card(tagID, title, BASE_URL, headers1)
#     title = "More Details"
#     hide_card(tagID, title, BASE_URL, headers1)
#     title = "Reviews"
#     hide_card(tagID, title, BASE_URL, headers1)
#     title = "Location"
#     print move_last(tagID, title, BASE_URL, headers1)
#     print halfwide_card(tagID, title, BASE_URL, headers1)
#     title = "Project Team"
#     print halfwide_card(tagID, title, BASE_URL, headers1)