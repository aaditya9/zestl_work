...
#     nextstages = result.split(";")
    ...
    try:
        ...
        for f_name in result['data']['elements']:
            if "Lead Initiation" in f_name['title']:
                data = json.loads(f_name['content'])
                for e1 in data['Elements'][0]['Elements']:
                    if "Employee Name" in e1['FieldLabel']:
...
def create_user_grps(body, flow, BASE_URL, headers1, parentID):
    ...
    try:
        newgrpID = CM.create_groups(grpname, flow + " group for project : " + body['FormData']['wid'], headers1, body['BusinessTag'], BASE_URL)
        mgrgrpID = CM.create_groups(mgrgroup, flow + " group for project : " + body['FormData']['wid'],headers1, body['BusinessTag'], BASE_URL)
    except:
...
def create_wfes(body, masterfileName, headers1, BASE_URL):
    ...
    try:
        ...
        for formname in result['data']['elements']:
            if "Lead Initiation" in formname['title']:
...
def user_addition_using_Form(body, BASE_URL, headers1):
    if body['FormData']['User ID'] is None:
        ...
        try:
            name = body['FormData']['HR Name']
            email = body['FormData']['HR Email']
            phone_number = body['FormData']['Phone Number']
            grp_name = "HRD"
        except:
            name = body['FormData']['Candidate Name']
            email = body['FormData']['Candidate Email']
            phone_number = body['FormData']['Phone Number']
            grp_name = "Candidate"
...
def topformSubmit(body, workflowTopFormID, headers1, BASE_URL):
    ...
    if body['FormData']['Date of Joining'] is None:
        ...
        try:
            try:
                ...
                lead = []
                lia_lead = body['FormData']['Candidate Name']
            ...
            if createdvalue == "113":
                ...
                if body['FormData']['Name'] != None :
                    # logging.warning("creation is start")
                    masterFileName = "SOLUTION REQUEST_list_new.csv"
                    create_wfes(body, masterFileName, headers1, BASE_URL)
            ...
            # logging.exception("error")
            CM.add_val_in_table(BASE_URL, headers1, formkey, 113, body['BusinessTag'],int(wid))
            if body['FormData']['Team'] == "Marketing" or body['FormData']['Team'] == "Sales" or body['FormData']['Team'] == "Accounts" \
                    or body['FormData']['Team'] == "HR":
                logging.warning("creation is start")
                masterFileName = "team_list.csv"
                create_wfes(body, masterFileName, headers1, BASE_URL)
                logging.warning("Creation is done")
...
def mainworkflow(body, h1, B_URL):
    ...
    filepath_server = "/var/www/cgi-bin/workflow/firesolution/"
    filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Prod_Server_WORKFLOW/adidev/"
    # filepath_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/Haldiram/wfe/"

    workflowTopFormID = "64"     ## Top Project management form TEST SERVER form ID
    # create_user_formID = "123"  #### user addition Candidate
    # create_user_formID_HR = "122"   # user addition HR
    # packagingform = "117"
    ...
    if body['Cmd'] == "form-submit":
        ...
        # elif body['FormID'] == create_user_formID_HR:
        #     user_addition_using_Form(body, BASE_URL, headers1)
