...
def start_workflow(workflowTopFormID, body, BASE_URL, headers1):
    outdict = {}
...
def read_usual_file(usuallistFile,usuallistFile_server):
    hasHeader = "Y"
    cardList = {}
...
def task_assignment_form(alltag,body,tagName,st,wfid,formname,headers1,BASE_URL):
    ...
    if flag is not 1:
        ...
        field_label_1['Apply to all tasks in this form (adding a name in other task will override that particular task assignment)'] = ""
...
def updateEstimated_SkipWorkflow(alltag,body,KEY,form_ID,BASE_URL,headers1):
    data = json.loads(alltag['content'])
    for subelm in data['Elements'][0]['Elements']:
        s_id = subelm['MetaData']['PYTHON']
        estimate_value = subelm['Value']
        input_data = {KEY: estimate_value}
        result = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1, form_ID, input_data, s_id)


def statusForm_active(tasktag,BASE_URL,body,form_ID,startExists,s_id,headers1):
    value = CM.get_all_tagIds(tasktag, BASE_URL, body['BusinessTag'], headers1)
    v = json.loads(value)
    taskID_array = []
    for t_val in json.loads(value)['data']['elements']:
...
def complete_wfe(tagName1, wfid, BASE_URL, body, headers1,filepath,filepath_server,cardList):
...
def create_usual_forms(val1, ele, body, BASE_URL, headers1, wfectr,filepath,filepath_server):
...
def create_worflow_elements(wfe, body, wfesparentID, filepath, filepath_server, BASE_URL, headers1, threadID, threads, wfectr, cardList, top_status):
    ...
    for ele in usualForms:
        create_usual_forms(val1, ele, body, BASE_URL, headers1, wfectr,filepath,filepath_server)
...
def create_user_grps(body, flow, BASE_URL, headers1, parentID):
    ...
    try:
        if s.createmgrgrp == True:
            mgrgrpID = CM.create_groups(mgrgroup, flow + " group for project : " + body['FormData'][s.Top_Form_Field],headers1, body['BusinessTag'], BASE_URL)
        else:logging.warning("MGR is Not set to TRUE")

        newgrpID = CM.create_groups(grpname, flow + " group for project : " + body['FormData'][s.Top_Form_Field], headers1, body['BusinessTag'], BASE_URL)
...
def create_CONFIG_TASK(lisval,wfe,parentID,topname,tagName, filepath, filepath_server, body, BASE_URL, headers1):
    tagT = ""
    ...
    elem = {}
    elem[
        'label'] = "Apply to all tasks in this form (adding a name in other task will override that particular task assignment)"
    elem['type'] = "AUTO_COMPLETE"
    elem['required'] = 0
    elem['placeholder'] = ""
    elem['editable'] = "true"
...
def create_config_ESTIMATE(lisval,wfe,parentID,body,topname,tagName,filepath,filepath_server,BASE_URL,headers1):
    e_arr = []
    fields = {}
    logging.warning("###%%%%%%%%%%#####")
...
def create_config_SKIP(lisval,wfe,parentID,body,topname,tagName,filepath,filepath_server,BASE_URL,headers1):
...
def create_config_cards(wfe, lisval, wfesparentID, parentID, filepath, filepath_server, body, BASE_URL, headers1):
    ...
    if s.onetime_permission_settings == True:
        for per in s.config_view_permissions:
    ...
    if s.create_task == True:
        logging.warning("*********************  Creating TASK Form  *************************")
        config_task = create_CONFIG_TASK(lisval,wfe,parentID,topname,tagName, filepath, filepath_server, body, BASE_URL, headers1)
    else:logging.warning("Please ignore TASK")
...
#*********************************************************************************************************************
    #####
    if s.create_estimate == True:
        logging.warning("*********************  Creating Estimate Form  ********************************")
        create_estimate = create_config_ESTIMATE(lisval,wfe,parentID,body,topname,tagName,filepath,filepath_server,BASE_URL,headers1)
    else:logging.warning("Please ignore ESTIMATE")

    if s.create_skip == True:
        logging.warning("*********************  Creating Skip Form  ************************")
        create_skip = create_config_SKIP(lisval,wfe,parentID,body,topname,tagName,filepath,filepath_server,BASE_URL,headers1)
    else:logging.warning("Please ignore SKIP")
...
def create_wfes(body, masterfileName, headers1, BASE_URL,filepath,filepath_server):
    ...
    standard_C_T_R = s.WFE_Cost_Time
    ...
    createFile = s.Create_logFile + body['WorkflowID'] + ".log"
    createFile_PKG = s.Create_PackagingLogFile + body['WorkflowID'] + ".log"
    ...
    try:
        ...
        for formname in result['data']['elements']:
            if s.Top_Form in formname['title']:
...
def check_Mandatory_checkBOX(checkboxes,checkreq,decisionname,wfidtag,formname,wfid,filepath,filepath_server,cardList,body,BASE_URL,headers1):
    flag = False
        ...
        if exitchecklistFlag:
            ...
            if "Exit Checklist" in formname:
                ...
                if completeFlag:
                    ...
                    logging.warning("Stages are not complete")

        else:
            tasktag = "task:" + decisionname + ":" + st + ":" + wfidtag
            completeFlag = check_alltasks_complete(tasktag, body, BASE_URL, headers1)
            if completeFlag:
                set_wfe_status(decisionname + ":" + wfidtag, "Completed", body, BASE_URL, headers1, cardList)

        if "Exit Checklist" in formname:
            logging.warning("HERE we found EXIT Checklist")
            tagName1 = decisionname + ":WFID_" + wfid
            set_wfe_status(tagName1, "Completed", body, BASE_URL, headers1, cardList)

        elif "Entry Checklist" in formname:
            tagName1 = decisionname + ":WFID_" + wfid
            set_wfe_status(tagName1, "Active", body, BASE_URL, headers1, cardList)

def Status_In_Formname(decisionname,body,st,wfidtag,tagName,cardList,BASE_URL,headers1,wfid,filepath,filepath_server):
    tasktag = "task:" + decisionname + ":" + st + ":" + wfidtag
    ...
    if "Completed" in status:
        complete_wfe(tagName, wfid, BASE_URL, body, headers1, filepath, filepath_server, cardList)
    elif status == "Active":
        statusForm_active(tasktag, BASE_URL, body, form_ID, startExists, s_id, headers1)


def link_WFES(body,BASE_URL,headers1,formname,decisionname):
    flag = "s"
                            ...
                            for work_id in wf_array:
                                ...
                                for sub_cardID in json.loads(result)['data']['elements']:
                                    if form in sub_cardID['Tags']:
                                        ...
                                        CM.EDIT_submission_using_NEW_API_using_ChangeIn_URL(BASE_URL, body['BusinessTag'],headers1, form_ID,input_data,submission_cardID)
...
def check_CHECK_BOX_true(tagName,formname,BASE_URL,body,headers1):
    checkreq = {}
    checkboxes = {}
    c_tag = tagName
    ...
    return checkreq,checkboxes

def packagingSubmit(body, BASE_URL, headers1, packagingform, filepath,filepath_server):
    if len(body['tags']) > 0:
...
def send_mail_notification(comm_TYPE,body,grpname,BASE_URL,headers1):
    mail = {}
    mail['title'] = "Project " + '"' + body['FormData'][s.Top_Form_Field] + '"' + s.msg_1
    mail['msg'] = "Project " + '"' + body['FormData'][s.Top_Form_Field] + '"' + s.msg_2
    mail['commtype'] = [comm_TYPE]
    mail['groupname'] = [grpname]
    method = "POST"
    url = BASE_URL + "usergroups/message/" + body['BusinessTag']
    resp = CM.hit_url_method(mail, headers1, method, url)
    logging.warning("sending proj completion Mail and Notification")
    logging.warning(resp)


def hide_user_card(headers1, BASE_URL, user_id):
...
def create_self_grp_and_ADDuser(name, user_id,BASE_URL, headers1,body):
...
def add_user_in_GivenInExcel(file_name,department,role,user_id,headers1,BASE_URL,body):
