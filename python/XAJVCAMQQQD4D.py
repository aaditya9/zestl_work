...
def add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1):
    f_title = "Hospital 1"
    brand_name = body['FormData']['Customer Name']
    b = {}
    url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/" + doctor_master
    method = "GET"
    ...
    f_title = "Referring hospital/laboratory/organization"
    url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/" + referral
    method = "GET"
    jsonresponse = CM.hit_url_method(b, headers1, method, url)
    result = CM.edit_form_spinner_element(BASE_URL, body['BusinessTag'], headers1, jsonresponse, f_title, brand_name)
    logging.warning("Spinner updated successfully")
...
def user_addition_using_Form(body, BASE_URL, headers1):
    ...
    if body['FormData']['User ID'] is None:
        ...
        if flag == 1:
            ...
            logging.warning("we are not setting Contact details here")

        if flag == 2:
            hasHeader = "Y"
            # file_name = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/GenePath_Mapping_of_role_usergroup.csv"
            file_name = "/var/www/cgi-bin/workflow/GenePath_Mapping_of_role_usergroup.csv"
            with open(file_name, 'r') as rf:
                data = csv.reader(rf, delimiter=',')
                if hasHeader == "Y":
                    row1 = data.next()
                for row in data:
                    if row[0].strip() in role:
                        grp_name = row[1].strip()
                        usergroups = CM.getAllUserGroups(headers1, body['BusinessTag'], BASE_URL, grp_name)
                        grpvals = json.loads(usergroups)
                        grps = grpvals['output']
                        grpname = grps['usergroup']
        ...
        else:logging.warning("Ignoring flag 2")
        ...
        if flag == 3:
            ...
            b = {}
            jsonresponse = CM.hit_url_method(b, headers1, method, url)
            result = CM.edit_form_spinner_element(BASE_URL, body['BusinessTag'], headers1, jsonresponse, f_title,name)
            logging.warning("Spinner updated successfully")
            logging.warning(result)
        else:logging.warning("Flag not found")
        ...
        logging.warning("User is already created")


def create_worflow_element_filechain(wfe, body, wfesparentID, wfectr, cardList, testname,next_stage):
    #
    filename = filepath + wfe.strip() + ".csv"
    filename_server = filepath_server + wfe.strip() + ".csv"
...
def create_wfes_filechain(body):
    ...
    standard_C_T_R = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/WFE_Time_Cost.csv"
    # standard_C_T_R_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/WFE_Time_Cost.csv"
    ...
    try:
        ...
        testNumber = body['FormData']['Test No.(Test Name)'].strip()
        testNumber = testNumber.split('(')
        ...
        for testno in json.loads(jsonresponse)['data']['result']:
            if testno['Test No.'] == testNumber:
                ...
                tat = testno['TAT(days)']
                additional_info_test_name = testno['Test Name']
    ...
    try:
        ...
        try:
            ...
            logging.warning(result)
            for f_name in json.loads(result)['data']['elements']:
                if "Additional Test Info" in f_name['title']:
                    sub_id = f_name['cardID']
                    # additional_test_info = 5312
                    ...
                    input_data = {"Test No": test_no, "Analytical Technique": analytical_teq,"Therapeutic area": therap_area, "Outsourced": outsourced,
                                  "NGS Capacity Reqd": ngs_capacity, "TAT": tat, "Test Name" : additional_info_test_name, "Test Performed" : testname}
                    result = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1,form_ID, input_data, sub_id)
                    logging.warning(result)
                elif "Billing" in f_name['title']:
                    sub_id = f_name['cardID']
                    # billing_form_id = 5323
                    ...
                    result = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1, form_ID,input_data, sub_id)
                    logging.warning(result)
                elif "Sample Reception" in f_name['title']:
                    sub_id = f_name['cardID']
            ...
            for f_name in json.loads(result)['data']['elements']:
                if "Sample Reception" in f_name['title']:
                    data_1 = json.loads(f_name['content'])
                    for elm_1 in data_1['Elements'][0]['Elements']:
                        if "Sample Received Date" in elm_1['ElementID']:
                            s_date = elm_1['Value']
                            logging.warning(s_date)
                elif "Additional Test Info" in f_name['title']:
                    data_1 = json.loads(f_name['content'])
                    for elm_1 in data_1['Elements'][0]['Elements']:
            ...
            abc = details[0].strip()
            for f_name in json.loads(result)['data']['elements']:
                if "Test Status" in f_name['title']:
                    sub_id = f_name['cardID']
...
#***************************************************************************************************************
    ...
    try:
        ...
        b['msg'] = "workflow step  " + val1['desc'] + " of workflow is completed"
        b['commtype'] = ["NOTIFY"]
...
def complete_wfe_old(tagName1, wfid, BASE_URL, body, headers1):
    # filepath = "/Users/sujoychakravarty/PycharmProjects/TwigMeScripts/python/wfes/"
    # filepath_server = "/var/www/cgi-bin/workflow/wfes/wfes_GenePath/"
...
#******************** commenting for some time   **************************#
    try:
        ...
        top_user = body['FormData']['Sales Lead']
        if top_user == '':
            logging.warning("empty field")
        else:
            logging.warning(top_user)
            result = CM.add_user_to_group(g_ID, top_user, body['BusinessTag'], headers1, BASE_URL)
            logging.warning(result)

        group = ["Lab_Operators","Lab_Managers","Lab_Doctors","Sample_Reception","Operations"]
        for inside_group in group:
            ...
            logging.warning(result)
    except:logging.warning("name not found")
    ...
    flag = "N"
    for f_name in json.loads(result)['data']['elements']:
        if "Patient Initiation" in f_name['title']:
            sub_id = f_name['cardID']
            data = json.loads(f_name['content'])
            for e1 in data['Elements'][0]['Elements']:
...
def create_config_cards(wfe, lisval, wfesparentID, parentID, filepath, filepath_server, body, BASE_URL, headers1):
    ...
    op_permission = ["Lab_Operators","Lab_Managers","Lab_Doctors"]
    for per in op_permission:
    ...
    try:
        ...
        for wfe in lisval:
            ...
            if wfe == "END":
                ...
                next_stage = wfe
                filename = filepath + next_stage.strip() + ".csv"
                filename_server = filepath_server + next_stage.strip() + ".csv"
                tag = next_stage.strip() + ":WFID_" + str(body['WorkflowID'])
...
def topformSubmit(body, workflowTopFormID, headers1, BASE_URL):
    ...
    #************************ commenting for some time *************************************************
                        ...
                        input_data = {"Patient Name": pname, "Patient Phone No": body['FormData']['Patient Phone No'],
                                      "Patient Email": body['FormData']['Patient Email'], "User ID": str(user_id),
                                      "Date of Birth": body['FormData']['Date of Birth'],
                                      "Age": body['FormData']['Age'], "Sex": body['FormData']['Sex']}
                        result = CM.form_submission_using_NEW_API_genepath(BASE_URL, body['BusinessTag'], headers1, form_ID,
                                                                  input_data)
                        logging.warning(result)
...
def mainworkflow(body, h1, B_URL):
    ...
    referral = "162"
    test_master_metadata = "175"
    additional_test_info = "163"
    billing_form_id = "152"
    sample_reception_form = "155"
    id_tracking = "158"
    patient_initiation_form = "6594"
    id_admin = "173"
    test_status_form = "161"
    patient_form = "1089"
    doctor_master = "1079"
    # patient_initiation_form = "153"


    filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/wfes/"
    filepath_server = "/var/www/cgi-bin/workflow/wfes_GenePath/"
    # filepath_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/wfes/"


    # workflowTopFormID = "153"    ### patient intimation form previous
    workflowTopFormID = "6594"  ### patient intimation form
    create_user_formID_DOCTOR = "1079"    # add user doctor
    create_user_formID_EMP = "1081"    # add user employee
    create_user_formID_PATIENT = "1089" # add user patient
    add_customer_to_doctor = "1077" #spinner add customer
    packagingform = "15150"
    ...
    if body['Cmd'] == "form-submit":
        ...
        if body['FormID'] == workflowTopFormID:
        ...
        elif body['FormID'] == add_customer_to_doctor:
            ...
            try:
                ...
                if "_CONFIG" in decisionname:
                ...
                elif "Test Selection" in formname:
                    ...
                    #************************************  We are not putting thiscode on prod. so we are commenting this code  ***********************
                        ...
                        hasHeader = "Y"
                        # mapping_TOP = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/PROD_Gene_Path/Mapping_GENEpath.csv"
                        ...
                        # result_1 = CM.get_all_tagIds(c_tag, BASE_URL, body['BusinessTag'], headers1)
                        for alltag in result['data']['elements']:
