        ...
        for f_name in result['data']['elements']:
            if "Project Management" in f_name['title']:
                ...
                for e1 in data['Elements'][0]['Elements']:
                    if "Token Number" in e1['FieldLabel']:
...
def create_user_grps(body, flow, BASE_URL, headers1, parentID):
    ...
    try:
        newgrpID = CM.create_groups(grpname, flow + " group for project : " + str(body['FormData']['Token Number']), headers1, body['BusinessTag'], BASE_URL)
        mgrgrpID = CM.create_groups(mgrgroup, flow + " group for project : " + str(body['FormData']['Token Number']),headers1, body['BusinessTag'], BASE_URL)
    except:
...
def create_wfes(body, masterfileName, headers1, BASE_URL):
    ...
    try:
        form_ID = client_master
        client_tag = body['FormData']['Client Name']
        # url = BASE_URL + "zvice/basecard/"  # Base card API
        # method = "POST"
        # b = {}
        # b['tagIds'] = [client_tag]
        # jasub = CM.hit_url_method(b, headers1, method, url)
        #
        # for result in json.loads(jasub)['data']['elements']:
        #     if client_tag == result['tagId']:
        #         user_name = result['title']

        field_key = "User ID"
        jsonresponse = CM.search_form_submission(BASE_URL, headers1, body['BusinessTag'], form_ID, field_key, client_tag)
        for testno in json.loads(jsonresponse)['data']['result']:
            if testno['User ID'] == client_tag:
                avail_credits = testno['Available Credits (In Minutes)']
                tag = "TOP:WFID_" + str(body['WorkflowID'])
                result = CM.get_all_tagIds(tag, BASE_URL, body['BusinessTag'], headers1)
                for f_name in json.loads(result)['data']['elements']:
                    if "Credit Consumed" in f_name['title']:
                        form_ID = credit_confused
                        sub_id = f_name['cardID']
                        input_data = {"Available Credit (In Minutes)": avail_credits}
                        sub = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1,form_ID, input_data, sub_id)

    except:logging.warning("not found")
...
def user_addition_using_Form(body, BASE_URL, headers1):
    if body['FormData']['User ID'] is None:
        ...
        try:
            name = body['FormData']['Client Name']
            field_name = body['FormData']
        except:
            name = body['FormData']['ENG Name']
            field_name = body['FormData']
...
def add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1):
    try:
        brand_name = body['FormData']['Product Name']
        field_name = body['FormData']
    except:
        brand_name = body['FormData']['New Product Category Name']
        field_name = body['FormData']
    arr = ["Product 1","Product 2","Product 3"]
    # for elm in arr:
    # #     f_title = "Brand"
    # # else:
    ...
    #
    #     b = {}
    #     url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/744"
    #     method = "GET"
    #     jsonresponse = CM.hit_url_method(b, headers1, method, url)
    #
    #     result = CM.edit_form_spinner_element(BASE_URL,body['BusinessTag'],headers1,jsonresponse,elm,brand_name)
    #     logging.warning("Spinner updated successfully")
    # #     logging.warning(result)

    b1 = {}
    # elm = "SER_WFE1_3:Check Out"
    # form_id = cardList[elm]
    form_id = check_out
    url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/" + form_id
    method = "GET"
    jsonresponse = CM.hit_url_method(b1, headers1, method, url)
    elm = "Product Consumed"
    result = CM.edit_form_spinner_element(BASE_URL, body['BusinessTag'], headers1, jsonresponse, elm, brand_name)
    logging.warning("Spinner updated successfully")
    logging.warning(result)
...
def topformSubmit(body, workflowTopFormID, headers1, BASE_URL):
    ...
    if body['FormData']['Prefered Date'] is None:
        ...
        try:
            ...
            #
                lead = []
                client_name = body['FormData']['Client Name']
                lead.append(client_name)
            ...
            if createdvalue == "113":
                logging.warning("resubmit")
                #
                # masterFileName = "service_wfe.csv"
                # create_wfes(body, masterFileName, headers1, BASE_URL)
                ...
                if body['FormData']['Service Request For'] == "Email" or body['FormData']['Service' \
                ' Request For'] == "Cloud" or body['FormData']['Service Request For'] == "Infrastructure" or body['FormData']['Service Request For'] == "Break Up":
            ...
            # logging.exception("error")
            CM.add_val_in_table(BASE_URL, headers1, formkey, 113, body['BusinessTag'],int(wid))
            if body['FormData']['Service Request For'] == "Email" or body['FormData']['Service Request For'
            ] == "Cloud" or body['FormData']['Service Request For'] == "Infrastructure" or body['FormData']['Service Request For'] == "Break Up":
                logging.warning("creation is start")
                masterFileName = "service_wfe.csv"
                create_wfes(body, masterFileName, headers1, BASE_URL)
                logging.warning("Creation is done")
...
def mainworkflow(body, h1, B_URL):
    ...
    global headers1
    global check_in
    global credit_confused
    global client_master
    global check_out
    global product_confused
    ...
    client_master = "740"
    check_out = "18"
    product_confused = "744"
    product_master = "742"

    # filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/6DEC_data_FG/"

    filepath_server = "/var/www/cgi-bin/workflow/ServiceDemo/"
    filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/Services Demo/"
    # filepath_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/Services Demo/"

    workflowTopFormID = "10"     ## Top Service Request form TEST SERVER form ID
    packagingform = "117"
    create_user_formID = "740"  # user addition form id
    add_spinner_element = "742" # spinner element
    create_engineer = "1088"    # user addtion engineer form id
    # assign_task_in_WFE_flow = "12"  #assign task in wfe flow
    ...
    if body['Cmd'] == "workflow-create":
        outdict = {}
        input_data = {"wid" : str(body['WorkflowID']),"Token Number" : "Token No: " + str(body['WorkflowID'])}
    ...
    if body['Cmd'] == "form-submit":
        ...
        if body['FormID'] == packagingform:
        ...
        elif body['FormID'] == create_engineer:
            user_addition_using_Form(body, BASE_URL, headers1)


        elif body['FormID'] == add_spinner_element:
            add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1)
            ...
            try:
                ...
                if "_CONFIG" in decisionname:
                    ...
                    try:
                        #*************************   15 Feb 12.25 new Code *****************************************
                        if formname == "Check Out":         # form name hard coded
                            check_form_ID = body['FormID']
                            check_sub_ID = body['FormSubmissionID']
                            check_out_pc1 = body['FormData']['Product Consumed 1']
                            check_out_pc2 = body['FormData']['Product Consumed 2']
                            check_out_pc3 = body['FormData']['Product Consumed 3']
                            check_out = {}
                            method = "GET"
                            url = BASE_URL + body['BusinessTag'] + "/forms/" + check_form_ID + "/submissions/" + check_sub_ID
                            jsonresponse = CM.hit_url_method(check_out, headers1, method, url)
                            result = json.loads(jsonresponse)['data']['elements']
                            for check in result:
                                if check['title'] == "Check Out":
                                    data = json.loads(check['content'])
                                    last_date = data['DateModified']
                            tag = "SER_WFE1_3:WFID_" + str(wfid) +  ",Check In"     # tag name hard coded
                            form = "Check In"   # form name hard coded
                            t_result = CM.combine_tag_gives_cardID(BASE_URL,body['BusinessTag'], headers1,tag)
                            for sub_cardID in json.loads(t_result)['data']['elements']:
                                if form in sub_cardID['Tags']:
                                    submission_cardID = sub_cardID['CardID']
                            # In_form_ID = "17"
                            In_form_ID = check_in
                            In_sub_ID = submission_cardID
                            check_IN = {}
                            method = "GET"
                            url = BASE_URL + body['BusinessTag'] + "/forms/" + In_form_ID + "/submissions/" + str(In_sub_ID)
                            jsonresponse = CM.hit_url_method(check_IN, headers1, method, url)
                            result = json.loads(jsonresponse)['data']['elements']
                            for check in result:
                                if check['title'] == "Check In":
                                    data = json.loads(check['content'])
                                    first_date = data['DateModified']
                            ...
                            val = d1_ts - d2_ts
                            diff_time = abs(val) / 60

                            diff_time = round(diff_time,0)
                            diff_time = int(diff_time)
                            logging.warning(diff_time)

                            top_tag_1 = "TOP:WFID_" + str(wfid)
                            result = json.loads(CM.get_all_tagIds(top_tag_1, BASE_URL, body['BusinessTag'], headers1))
                            for formname in result['data']['elements']:
                                if "Credit Consumed" in formname['title']:
                                    top_sub_id = formname['cardID']
                                    data = json.loads(formname['content'])
                                    for el in data['Elements'][0]['Elements']:
                                        if "Available Credit (In Minutes)" in el['FieldLabel']:
                                            ac_time = el['Value']
                                            ac_time = int(ac_time)
                                            total_ac_time = ac_time - int(diff_time)
                                        # if "Credit Consumed (In Minutes)" in el['FieldLabel']:
                                        #     cc_time = el['Value']
                                        #     cc_time = int(cc_time)
                                        #     total_cc_time = cc_time + diff_time
                                    # form_ID = "741"
                                    form_ID = credit_confused
                                    input_data = {"Available Credit (In Minutes)" : str(total_ac_time), "Credit Consumed (In Minutes)" : str(diff_time)}
                                    result = CM.EDIT_submission_using_NEW_API(BASE_URL,body['BusinessTag'],headers1,form_ID,input_data,top_sub_id)
                            ...
                            for formname in result['data']['elements']:
                                if "Project Management" in formname['title']:
                                    data = json.loads(formname['content'])
                                    for el in data['Elements'][0]['Elements']:
                                        if "Client Name" in el['FieldLabel']:
                                            c_name = el['Value']
                                            # result = CM.tags_give_base_cardInfo(BASE_URL,c_name,headers1)
                                            # for jasub in json.loads(result)['data']['elements']:
                                            #     if c_name == jasub['tagId']:
                                            #         user_name = jasub['title']
                                            field_key = "User ID"
                                            form_ID = client_master
                                            result_1 = CM.search_form_submission(BASE_URL,headers1,body['BusinessTag'],form_ID,field_key,c_name)
                                            for client_c in json.loads(result_1)['data']['result']:
                                                if client_c['User ID'] == c_name:
                                                    avail_credits = client_c['Available Credits (In Minutes)']
                                                    sub_ID = client_c['FormSubmissionID']
                                                    final_credit = int(avail_credits) - int(diff_time)
                                                    input_data = {"Available Credits (In Minutes)": str(final_credit)}
                                                    sub = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1, form_ID, input_data,sub_ID)

                                if "Product Consumed" in formname['title']:
                                    PC_sub_id = formname['cardID']
                                    form_ID = product_confused
                                    field_key = "Product Name"
                                    f_value = check_out_pc1
                                    c_result = CM.search_form_submission(BASE_URL, headers1, body['BusinessTag'], product_master, field_key, f_value)
                                    for cust_price in json.loads(c_result)['data']['result']:
                                        if cust_price['Product Name'] == f_value:
                                            customer_price_1 = cust_price['Customer Price (Before Tax)']

                                    f_value = check_out_pc2
                                    c_result = CM.search_form_submission(BASE_URL, headers1, body['BusinessTag'],product_master, field_key, f_value)
                                    for cust_price in json.loads(c_result)['data']['result']:
                                        if cust_price['Product Name'] == f_value:
                                            customer_price_2 = cust_price['Customer Price (Before Tax)']

                                    f_value = check_out_pc3
                                    c_result = CM.search_form_submission(BASE_URL, headers1, body['BusinessTag'],product_master, field_key, f_value)
                                    for cust_price in json.loads(c_result)['data']['result']:
                                        if cust_price['Product Name'] == f_value:
                                            customer_price_3 = cust_price['Customer Price (Before Tax)']

                                    input_data = {"Product1":check_out_pc1,"Product 1 Price": str(customer_price_1),
                                                  "Product2":check_out_pc2,"Product 2 Price": str(customer_price_2),
                                                  "Product3":check_out_pc3,"Product 3 Price": str(customer_price_3)}
                                    result = CM.EDIT_submission_using_NEW_API(BASE_URL, body['BusinessTag'], headers1,form_ID, input_data, PC_sub_id)
                                    # except:logging.warning("No entry found in PRODUCT MASTER..... Please update the field and try again")


                        else:logging.warning("Not found")
                        #*******************************************************************************************
                        stage_form_id = body['FormID']
                        stage_form_id = "FormID_" + str(stage_form_id)
                        createdvalue = CM.get_formID_using_KEY_Value_API(BASE_URL, headers1, body['BusinessTag'],stage_form_id)
                        try:
                            createdvalue = json.loads(createdvalue)
                            createdvalue = createdvalue['data']['value']['_value']
                            if createdvalue == formname:

                                task_filename_server = filepath_server + str(body['BusinessTag']) + "_stage.csv"
                                # task_filename_server = filepath + str(body['BusinessTag']) + "_stage.csv"

                                with open(task_filename_server, 'r') as rf:
                                    data = csv.reader(rf, delimiter=',')
                                    if hasHeader == "Y":
                                        row1 = data.next()
                                    for row in data:
                                        if "Assign Task" == row[2]:
                                            assign = body['FormData'][str(row[1])]
                                            grp_id = json.loads(assign)
                                            grp_id_1 = grp_id['GroupIDs']

                                            if formname in row[0]:
                                                t_tag = row[3]
                                                t_tag = t_tag.split(":")
                                                # sub_tag = "SER_WFE1_2:WFID_19","Schedule Service Call"
                                                # sub_tag = t_tag[0] + ":WFID_" + str(wfid) + "," + t_tag[1]
                                                sub_tag = t_tag[0] + ":WFID_" + str(wfid)
                                                result = CM.combine_tag_gives_cardID(BASE_URL, body['BusinessTag'], headers1, sub_tag)
                                                for sub_cardID in json.loads(result)['data']['elements']:
                                                    if t_tag[1] in sub_cardID['Tags']:
                                                        t_cardID = sub_cardID['CardID']
                                                        result = CM.get_all_tasks_on_cardID(BASE_URL,body['BusinessTag'],headers1,t_cardID)
                                                        for task_id in json.loads(result)['data']['elements']:
                                                            t_ID = task_id['cardID']
                                                            method = "POST"
                                                            url = BASE_URL + "tasks/edit/" + body['BusinessTag']
                                                            ...
                                                            for g_ID in grp_id_1:
                                                                ...
                                                                top_grp = "TOPp_" + str(wfid)
                                                                grp_in_grp_ID = CM.find_out_grp_ID(top_grp,headers1,body['BusinessTag'],BASE_URL)
                                                                by = {'grpUserGroupID': str(g_ID)}
                                                                result = CM.add_usergrps_grps(BASE_URL, json.dumps(by),headers1,body['BusinessTag'], grp_in_grp_ID)
                                                                logging.warning("Adding group to group")
                                                                logging.warning(result)

                                                    else:logging.warning("Tag not found")
                                        else:logging.warning("Action type is different")

                            else:logging.warning("not able to find")
                        except:logging.warning("Form KEY not found")
