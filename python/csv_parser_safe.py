...
def returnBaseLevel(base_column):
    ...
    for i in base_column:
        ...
        if (foundbase == 1 and 'tag' in i.lower()):  ### the tag ID of the organization
            zbotID = re.sub(r'(\w{13}).*\(\wag\).*', r'\1', i, flags=re.IGNORECASE)
        ...
        if ('card' in i.lower() and baseset == 1 and cardstarted == 0):  # brand new card
            isdept = 0
            cardType[numcards] = re.sub(r'(\w+)\s+\ward.*', r'\1', i, flags=re.IGNORECASE)  # cardtype
            ...
            isdept = 0
            cardType[numcards] = re.sub(r'(\w+)\s+\ward.*', r'\1', i, flags=re.IGNORECASE)
            endCard = rownum - 1
            ...
            zbotdep = re.sub(r'(\w{13}).*\(\wag\).*', r'\1', i, flags=re.IGNORECASE)
