            ...
            if "END" in stage:
                ...
                if EntryFlag:
                    ...
                    for status_card in json.loads(result)['data']['elements']:
                        if "Status Form F5" in status_card['allTags']:
    ...
    try:
        ...
        try:
            ...
            # result = json.loads(result['data']['elements'][0]['content'])
                    for e1 in data['Elements'][0]['Elements']:
                        if "Project Priority" in e1['FieldLabel']:
                            priority = e1['Value']
...
def add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1):
    ...
    b = {}
    url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/94847"   # Dev server forn id
    # url = BASE_URL + "all_actions/" + body['BusinessTag'] + "/form/77033"       # prod server form id
    method = "GET"
...
def assign_task_to_user(complete_adhoc_task, body, BASE_URL, headers1):

    form_ID = complete_adhoc_task
    input_data = {}
...
def mainworkflow(body, h1, B_URL):
    ...
    filepath_server = "/var/www/cgi-bin/workflow/wfes/"
    filepath_server = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/6DEC_data_FG/"

    # logFilePath = "/Users/sujoychakravarty/PycharmProjects/TwigMeScripts/logs/future/"
    # logFilePath_server = "/var/www/cgi-bin/workflow/logs/future/"


    # workflowTopFormID = "77033"     ## Top Project management form PROD SERVER form ID
    workflowTopFormID = "77390"     ## top Project Management form recently created DEV form ID
    packagingform = "15150"
    create_user_formID = "63881"    # this form is form Create user DEV
    # create_user_formID = "92411"  # this form is form Create user Prod
    add_users_to_group = "91780"    # this form id is for AddUser to group DEV
    # add_users_to_group = "92416"  # this form id is for AddUser to group prod
    add_brand = "91718"             # this form is for adding brand in PM form DEV
    # add_brand = "92413"             # this form is for adding brand in PM form Prod
    add_product = "91717"           # this form is for adding Product category in PM form DEV
    # add_product = "92412"   # this form is for adding Product category in PM form Prod
    adhoc_task = "91733"            # this form is for assign single task to grp DEV
    # adhoc_task = "92414"  # this form is for assign single task to grp Prod
    complete_adhoc_task = "95600"   # this form is for complete the status of adhoc task DEV
    # complete_adhoc_task = "92415"  # this form is for complete the status of adhoc task Prod
    ...
    if body['Cmd'] == "form-submit":
        ...
        if body['FormID'] == packagingform:
        ...
        elif body['FormID'] == add_users_to_group:
            add_single_user_to_givenGroups(body, BASE_URL, headers1)

        elif body['FormID'] == add_brand:
            add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1)

        elif body['FormID'] == add_product:
            add_SPINNER_elements_IN_FORM(body, BASE_URL, headers1)

        elif body['FormID'] == adhoc_task:
            assign_task_to_user(complete_adhoc_task, body, BASE_URL, headers1)

        elif body['FormID'] == complete_adhoc_task:
            complete_Adhoc_TASK(body, BASE_URL, headers1)
            ...
            try:
                ...
                elif "Status Form F5" in formname:
                        ...
                        for alltag in result['data']['elements']:
                            if formname in alltag['allTags']:
                                ...
                                #************************ Minal code ** Code for attachment********************************
                                 ...
                                 #**********************************************************
                                    elif "CHECK_BOX" in subelm['ElementType']:
