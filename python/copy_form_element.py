...
#outfile =path + 'form_tables.csv"


def copy_main(path,id):
    filename = path + 'form_id.csv'
    logging.warning(filename)
    filepath = path
    outfile = path + 'form_tables.csv'
    logging.warning(outfile)
    zviceID=id
    with open(outfile, 'w') as of:
        logging.warning("writing in form_tables")
...
if __name__ == "__main__":
    logging.warning("before calling copy_main func")
    copy_main(path,id)
    logging.warning("after calling copy_main func")
