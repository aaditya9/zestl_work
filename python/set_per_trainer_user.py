    ...
    with open(zbotFile, 'r') as rf:
        ...
        for row in data:
            ...
            for a in jsondata11['data']['elements']:
                title = "About Me!"
                if title in a['title']:
                    # parentCardID = json.loads(a['ctjsondata'])['cardID']
                    # url = a['cturl']
                    # body = json.loads(a['ctjsondata'])
                    # method = "POST"
                    print "Found 1st level"
                    # data1 = json.loads(a['data']['elements'])
                    # pid = a['cardID']
                    # print pid
                    # method = "POST"
                    # url = a['homeurl']
                    ...
                    for ac in a['data']['elements']:
                        if "About Me!" in ac['title']:
                            print "2nd level"
                                ...
                                if title in subac['title']:
                                    ...
                                    for subac1 in subac['actions']:
                                        ...
                                        if title in subac1['title']:
                                            ...
                                            pid = data1['cardID']
                                            print pid
                                            # response = set_card_permissions("Linked User", pid, zviceID, "ADMIN",headers1, "8SFBUKFZCALEE", BASE_URL)
                                        # print response
                                            # see whatsapp
