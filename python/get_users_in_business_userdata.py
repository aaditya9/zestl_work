...
if __name__ == '__main__':
    ...
    inputFile = "C:\Users\Minal Thorat\Dropbox\Zestl-scripts\millennium\script_inputs\QR_user.csv"
    outFile = "C:\Users\User\Dropbox\Zestl-scripts\millennium\script_inputs\uservrushali_OR.csv"
    hasHeader = "N"
    with open(outFile, 'w') as fw:
        with open(inputFile, 'r') as datalist:
            data = csv.reader(datalist, delimiter=',')
            if hasHeader == "Y":
                row1 = data.next()
            for row in data:
                writeString = ""
                for item in row:
                    writeString += item + ","
                writeString += "\n"
                writeString = details1[row[0]] +  "," + writeString
                fw.write(writeString)
                # print ":)"
                # print "found" + titlename
