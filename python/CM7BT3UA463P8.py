...
def create_wfes(body, masterfileName, headers1, BASE_URL):
    ...
    createFile = "/var/www/cgi-bin/workflow/Project_Demo_wfes/log/wfcreate_" + body['WorkflowID'] + ".log"
    createFile_PKG = "/var/www/cgi-bin/workflow/Project_Demo_wfes/log/wfcreate_PKG_" + body['WorkflowID'] + ".log"
    ...
    if body['FormData']['Project Title'] is None:
        ...
        try:
            try:
                ...
                lead = []
                coe_lead = body['FormData']['Commercial']
                lead.append(coe_lead)
                npd_lead = body['FormData']['CPC']
                lead.append(npd_lead)
                p_lead = body['FormData']['Design']
                lead.append(p_lead)
                mkt_lead = body['FormData']['Execution']
                lead.append(mkt_lead)
                pkg_lead = body['FormData']['Property']
            ...
            # logging.exception("error")
            CM.add_val_in_table(BASE_URL, headers1, formkey, 113, body['BusinessTag'],int(wid))
            if body['FormData']['Project Type'] == "Big Bazar" or body['FormData'
            ]['Project Type'] == "Central" or body['FormData']['Project Type'] == "FBB" or body['FormData']['Project Type'] == "HyperCity":
...
def mainworkflow(body, h1, B_URL):
    ...
    # filepath_server = "/var/www/cgi-bin/workflow/wfes/"
    filepath_server = "/var/www/cgi-bin/workflow/Project_Demo_wfes/"

    workflowTopFormID = "138"     ## Top Project management form TEST SERVER form ID
    packagingform = "117"
