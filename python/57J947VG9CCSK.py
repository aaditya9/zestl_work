...
#     nextstages = result.split(";")
    ...
    try:
        ...
        for f_name in result['data']['elements']:
            if "Trip Initiation" in f_name['title']:
                data = json.loads(f_name['content'])
                for e1 in data['Elements'][0]['Elements']:
                    if "Transaction ID" in e1['FieldLabel']:
...
def check_allStatus_complete(combine_tag,body,headers1,BASE_URL):
    result = CM.combine_tag_gives_cardID(BASE_URL, body['BusinessTag'], headers1, combine_tag)
    completeStatus = True
    for ctg in json.loads(result)['data']['elements']:
        sub_ID = ctg['CardID']
        result_1 = CM.get_form_submission(str(sub_ID), body['BusinessTag'], BASE_URL, headers1)
        for ele in json.loads(result_1)['data']['ondemand_action'][0]['inputs']:
            if "SuperSpinner" in ele['widget']:
...
def create_user_grps(body, flow, BASE_URL, headers1, parentID):
    ...
    try:
        newgrpID = CM.create_groups(grpname, flow + " group for project : " + body['FormData']['Transaction ID'], headers1, body['BusinessTag'], BASE_URL)
        mgrgrpID = CM.create_groups(mgrgroup, flow + " group for project : " + body['FormData']['Transaction ID'],headers1, body['BusinessTag'], BASE_URL)
    except:
...
def create_wfes(body, masterfileName, headers1, BASE_URL):
    ...
    try:
        ...
        for formname in result['data']['elements']:
            if "Trip Initiation" in formname['title']:
...
def topformSubmit(body, workflowTopFormID, headers1, BASE_URL):
    ...
    if body['FormData']['Vehicle Number'] is None:
        ...
        try:
            ...
            if createdvalue == "113":
                ...
                if body['FormData']['Trip From'] == "DC" or body['FormData']['Trip From'] == "FFCC1" or body[
                    'FormData']['Trip From'] == "FFCC2" or body['FormData']['Trip From'] == "FFCC3" or body['FormData']['Trip From'] == "BB1":
                    # logging.warning("creation is start")
                    masterFileName = "trip_wfe.csv"
                    create_wfes(body, masterFileName, headers1, BASE_URL)
            ...
            # logging.exception("error")
            CM.add_val_in_table(BASE_URL, headers1, formkey, 113, body['BusinessTag'],int(wid))
            if body['FormData']['Trip From'] == "DC" or body['FormData']['Trip From'] == "FFCC1" or body['FormData'][
                'Trip From'] == "FFCC2" or body['FormData']['Trip From'] == "FFCC3" or body['FormData']['Trip From'] == "BB1":
...
def mainworkflow(body, h1, B_URL):
    ...
    global BASE_URL
    global headers1
    global position_detailed

    BASE_URL = B_URL
    headers1 = h1
    position_detailed = "31"
    # filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/6DEC_data_FG/"

    # filepath_server = "/var/www/cgi-bin/workflow/wfes/"
    filepath = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/farmfresh/"
    filepath_server = "/var/www/cgi-bin/workflow/farmfresh/"

    workflowTopFormID = "8"     ## Top Project management form TEST SERVER form ID
    packagingform = "117"
    ...
    if body['Cmd'] == "workflow-create":
        outdict = {}
        input_data = {"wid" : str(body['WorkflowID']),"Transaction ID" : "FF" + str(body['WorkflowID'])}
            ...
            try:
                ...
                if "_CONFIG" in decisionname:
                    ...
                    if "Completed" in status:
                        ...
                        combine_tag = "Status" + "," + "DOCSEARCH::WFID_" + str(wfid)
                        completeStatus = check_allStatus_complete(combine_tag,body,headers1,BASE_URL)
                        if completeStatus:
                            tag = "TOP:WFID_" + str(wfid)
                            result = json.loads(CM.get_all_tagIds(tag, BASE_URL, body['BusinessTag'], headers1))
                            for f_name in result['data']['elements']:
                ...
                #******************************************************************************************************************************************************************
                        # Todays new code (13FEb 2018 time- 5.59)
                        # hasHeader = "Y"
                        # mapping_TOP = "C:/Users/Minal Thorat/MINAL OFFICE DATA/Hardik/Test_Server_workflow/farmfresh/TrialMapping_FarmFresh.csv"
                        # # mapping_TOP = "/var/www/cgi-bin/workflow/TrialMapping_FarmFresh.csv"
                        # # position_detailed = 31   #we are changing value only in one form. thats  why we hard coded this form id.
                        # form_ID = position_detailed
                        # c_tag = tagName
                        ...
                        #                         print subelm['ElementID']
                        #                         print row[4]
                        #                         if subelm['Value'].strip() == row[4].strip():
                        #
                        #                             val = result['data']['elements'][currentIdx]
                        #                             data_1 = json.loads(val['content'])
                        #                             for elm_1 in data_1['Elements'][0]['Elements']:
                        #                                 if row[5].strip() in elm_1['ElementID'].strip():
                        #                                     quantity = elm_1['Value']
                        ...
                        #                                 if row[7] in f_name['title']:
                        #                                     sub_ID = f_name['cardID']
                        #                                     input_data = {row[8]: quantity}
                        #                                     logging.warning(input_data)
                        #                                     result = CM.EDIT_submission_using_NEW_API(BASE_URL,body['BusinessTag'],headers1, form_ID,input_data, sub_ID)
                        #                                 else:
                        #                                     logging.warning("Title not found")
                        #                         else:
                        #                             logging.warning("Value not matching")
                        #             else:
                        #                 logging.warning("Form not found")
