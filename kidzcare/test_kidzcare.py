            ...
            for a in card['actions']:
                ...
                if json.loads(jsonresponse)['data']['elements']:
                    for info in json.loads(jsonresponse)['data']['elements']:
                        ...
                        if (doc_s_date == s_date) and (doc == json.loads(data)['tags']) and (
                                (doc_s_time == s_time) and (doc_e_time == e_time)):  # if date and doc sould be same
                            return False
                        else:
                            flag = True
                else:
...
# print dept_tagid
                logging.warning(dpt_id)
                url = LL.BASE_URL + 'genericcards/' + dpt_id
                body = {}
                method = "POST"
                # flag=True
